...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($selected_bname) && !empty($selected_date)) {
        // Fetch all due_date entries from the customer table for the selected branch
        $stmt = $con->prepare("SELECT id, due_date FROM customer WHERE bname = ?");
        $stmt->bind_param("s", $selected_bname);
        $stmt->execute();
        ...
        if (!empty($customers)) {
            ...
            echo "No customer found for the selected branch.<br>";
        }

        // Fetch all due_date_weekly entries from the paymentdates table for the selected branch from the selected date onwards
        $stmt = $con->prepare("
            SELECT id, due_date_weekly 
            FROM paymentdates 
            WHERE bname = ? AND (due_date_weekly = ? OR (due_date_weekly > ? AND MOD(DATEDIFF(due_date_weekly, ?), 7) = 0))
        ");
        $stmt->bind_param("ssss", $selected_bname, $selected_date, $selected_date, $selected_date);
        $stmt->execute();
        ...
        if (!empty($paymentdates)) {
            ...
            echo "No payment dates found for the selected branch and date.<br>";
        }

        // Insert the holiday into the holiday table
        $insert_holiday_stmt = $con->prepare("INSERT INTO holiday (name, date, cname) VALUES (?, ?, ?)");
        $insert_holiday_stmt->bind_param("sss", $holiday_name, $selected_date, $selected_bname);
        ...
        echo "Branch and date are required.<br>";
    }
}
