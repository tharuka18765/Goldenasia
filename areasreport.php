...
$default_date_from = date("Y-m-d", strtotime("-365 days"));
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM customer WHERE loan_date <= ? and loan_balance !=0 ";
    ...
    foreach ($results as $row) {
        ...
        $sql = "SELECT SUM(arriarse) AS arriarse FROM paymentdates WHERE customer_code = ? AND due_date_weekly BETWEEN ? AND ?";
        if ($stmt = $con->prepare($sql)) {
            $stmt->bind_param("sss", $customer_code, $selected_date_from, $selected_date_to);
            $stmt->execute();
            $result = $stmt->get_result();
            $arrears[$customer_code] = $result->fetch_assoc()['arriarse'];
            $stmt->close();
        }
    }
}
...
<h2>Arriarse Details</h2><div class="user-details">
<hr>
...
<div>
    ...
    <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
        <div>
            ...
            <input type="date" name="date_to" id="date_to" value="<?php echo htmlspecialchars($default_date_to); ?>" required>
            <br>
...
<?php if (!empty($results)): ?>
    ...
    <table border="1">
        ...
        <?php foreach ($results as $row): ?>
            <?php if (!empty($arrears[$row['customer_code']]) && $arrears[$row['customer_code']] > 0): ?>
                <?php 
                    $arrears_week = $arrears[$row['customer_code']] / $row['week_payment']; 
