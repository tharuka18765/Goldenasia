...
function getCustomerByNIC($con, $nic) {
    ...
    // Fetch customer information only if loan_balance is not 0
    $stmt = $con->prepare("SELECT name, cname, customer_code, nic, loan_code, loan_amount, full_loan, loan_balance, loan_date, week_payment, due_date 
                           FROM customer 
    ...
    if ($customer = $result->fetch_assoc()) {
        ...
        // Fetch arriarse and other details from paymentdates using the customer's loan_code
        ...
        if ($arriarseRow = $arriarseResult->fetch_assoc()) {
            ...
            $customerData['due_date_weekly'] = $arriarseRow['due_date_weekly'];
            $customerData['loan_code'] = $arriarseRow['loan_code'];
        } else {
...
// Handle AJAX request for searching customer by NIC
if (isset($_POST['action']) && $_POST['action'] == 'searchCustomer') {
    $nic = $_POST['nic'];
    $customer = getCustomerByNIC($con, $nic);

    if (!empty($customer)) {
        $customer['weekly_payments'] = getWeeklyPaymentsForCustomer($con, $customer['loan_code']);
    }
...
// Function to fetch all weekly payments for a customer
function getWeeklyPaymentsForCustomer($con, $loan_code) {
    $weeklyPayments = [];

    $stmt = $con->prepare("SELECT week_payment, week_number, arriarse, due_date_weekly 
                           FROM paymentdates 
                           WHERE loan_code = ?");
...
if (isset($_POST['action']) && $_POST['action'] == 'submitPayment') {
    ...
    try {
        // Fetch current loan_amount, total payment made so far, and NIC, but only if loan_balance is not 0
        $stmt = $con->prepare("SELECT loan_amount, full_loan, payment, nic, loan_date 
                               FROM customer 
                               WHERE loan_code = ? AND loan_balance != 0");
        ...
        if ($result->num_rows > 0) {
            ...
            $fetchStmt = $con->prepare("SELECT week_payment, due_date_weekly 
                                        FROM paymentdates 
                                        WHERE loan_code = ?");
            ...
            if ($fetchRow = $fetchResult->fetch_assoc()) {
                $weekDue = $fetchRow['due_date_weekly'];
            ...
            $arriarseStmt = $con->prepare("SELECT arriarse 
                                           FROM paymentdates 
                                           WHERE loan_code = ? AND week_number = ?");
            ...
            $newBalance = $fullLoan - $newPayment;

            $newArriarse = $weekPayment - $paymentAmount + $previousArriarse;

            // Update the customer table
            $updateStmt = $con->prepare("UPDATE customer 
                                         SET payment = ?, loan_balance = ?, payment_date = CURDATE() 
                                         WHERE loan_code = ?");
            ...
            $updatePaymentDatesStmt = $con->prepare("UPDATE paymentdates 
                                                     SET payment = ?, arriarse = ?, name = ?, payment_date = ? 
                                                     WHERE loan_code = ? AND week_number = ?");
            $updatePaymentDatesStmt->bind_param("ddsssd", $paymentAmount, $newArriarse, $executiveName, $dueDate, $loanCode, $weekNumber);
            ...
            echo json_encode(['success' => false, 'message' => 'Loan code not found or balance is 0']);
        }
    } catch (Exception $e) {
...
<div class="header-container">
    <div>
    <h3><?php echo htmlspecialchars($branchName); ?> Branch - Week Payments -  <span><?php
...
<div class="total-payment">
    ...
    <span id="total-payment-amount">Rs. 0.00</span>
</div>
...
$(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(customer) {
                    if (customer && customer.nic) {
                        var tableRows = '';
                        customer.weekly_payments.forEach(function(week) {
                            tableRows += `<tr>
                                ...
                                <td>${week.due_date_weekly}</td>
                                <td>${week.arriarse}</td>
                                <td>${customer.status || 'Not available'}</td>
                            </tr>`;
                        });
                        $('#search-results-table tbody').html(tableRows);
                    } else {
    ...
    var totalPayment = 0; // Initialize total payment amount
    var paymentsSubmitted = false; // Flag to track if any payments were submitted

    // Calculate total payment amount
    $('#search-results-table tbody tr').each(function() {
        var paymentAmount = $(this).find('.payment-input').val();
        if (paymentAmount) {
            totalPayment += parseFloat(paymentAmount); // Add payment amount to total payment
        }
    });

    // Update the total payment amount in the UI
    $('#total-payment-amount').text('Rs. ' + totalPayment.toFixed(2));

    // Display a confirmation popup before submitting payments
    var confirmed = confirm('Are you sure you want to submit payments? Total payment: Rs. ' + totalPayment.toFixed(2));
    if (!confirmed) {
    ...
    // Submit payments
    $('#search-results-table tbody tr').each(function() {
        var loanCode = $(this).find('td:eq(0)').text();
        var weekNumber = $(this).find('td:eq(5)').text();
        var duedate = $(this).find('td:eq(9)').text();
        var paymentAmount = $(this).find('.payment-input').val();

        // Check if payment amount is entered for the current row
        if (paymentAmount) {
            $.ajax({
                type: 'POST',
                url: 'testpay.php',
                ...
                success: function(response) {
                    if (response.success) {
                        paymentsSubmitted = true; // Set flag to true if any payment is successfully submitted
                        $(this).find('td:eq(6)').text(response.updatedBalance);
                        $('.payment-input').val('');
                    } else {
                        alert(response.message); // Show error message for failed payment
                    }
                },
                error: function(xhr, status, error) {
                    console.error('AJAX Error:', status, error);
    ...
    // Show success message if any payments were submitted successfully
    if (paymentsSubmitted) {
        alert('Payments updated successfully. Total payment: Rs. ' + totalPayment.toFixed(2));
    }
});
...
function validateInput(input) {
    input.value = input.value.replace(/[^\d.]/g, ''); // Allow only digits and decimal point
}
