<?php
session_start();
require "connecton.php"; // Ensure the file name is correct
...
if ($status == 'admin') {
    $branchQuery = "SELECT DISTINCT bname, bcode FROM branch ORDER BY bname";
...
if (isset($_POST['submit'])) {
    ...
    $query = "INSERT INTO customer (bname, bcode, center, ccode, `group`, name, cname, nic, address, phone1, phone2, image, loan_amount, type, interest, period, customer_code, loan_date, loan_code, due_date, payment, payment_date, full_loan, week_payment, document_fee, insurance_fee) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    
    $payment = 0.00;
    $paymentDate = date('Y-m-d');
    
    $stmt = mysqli_prepare($con, $query);
    mysqli_stmt_bind_param($stmt, "ssssssssssssssssssssssssss", $bname, $bcode, $center, $ccode, $group, $executiveName, $customerName, $customer_code, $address, $phone1, $phone2, $imageFileName, $loanAmount, $type, $interest, $period, $customer_code, $selectedDate, $loanCode, $dueDate, $payment, $paymentDate, $fullLoanAmount, $weeklyPayment, $documentfee, $insurancefee);
    ...
    $numberOfPayments = $period == '12' ? 12 : 13;
    ...
    for ($week = 1; $week <= $numberOfPayments; $week++) {
        ...
        $stmt = mysqli_prepare($con, $query);
        mysqli_stmt_bind_param($stmt, "ssissssssssss", $loanCode, $customer_code, $week, $dueweek, $bname, $center, $ccode, $payment, $customer_code, $name, $payment_date, $weeklyPayment, $arriarse);
        mysqli_stmt_execute($stmt);
...
body > div > section > form > div:nth-child(17) > button{
    margin-left:50px;
    width:200px;
    font-weight:500;
...
body > div > section > form > div:nth-child(17) > button:hover{
...
body > div > section > form > div:nth-child(17) > div > button{
    margin-left:300px;
    width:50%;
    font-weight:500;
    text-transform:uppercase;
...
body > div > section > form > div:nth-child(17) > div > button:hover {
    background-color: black;
    color:white;
}
</style>
...
<hr>
    ...
    <form action="old.php" method="post" enctype="multipart/form-data">
...
<div class="form-group">
    <label for="date"style="font-weight:bold; font-family:Arial;">Date</label>
...
<div class="form-group">
        ...
        <input type="tel" name="phone1" placeholder="Phone Number ">
</div>
<div class="form-group">
            <label for="center"style="font-weight:bold; font-family:Arial;">Customer Mobile Number</label>
